...
| [04-card-editor.md](04-card-editor.md) | SillyTavern 角色卡编写器（全屏浮层，16 个选项卡） |
...
│                               ⚠ 真品判据：**1 646 933 字节 / 31 747 行**（`sha1 1a071b564f4c…`）。
│                                 行数是 `grep -c ''` 口径（末行 `</html>` 后没有换行，所以 `wc -l` 会报 31 460）。
│                                 旁边那几份「副本」字节数都很接近，别靠文件名认。
│                                 ⚠ **这个数会漂** —— 本行换过好几轮
│                                    （`1 467 029 / 65df7b0b` → `1 487 774 / ecb46ca8de99`
│                                     → `1 493 542 / d0bf0d34` → `1 495 761 / 98ecb1b1`
│                                     → `1 500 654 / e3a3797c` → `1 502 845 / 9293421443`
│                                     → `1 514 918 / f26eb8c02a9e` → `1 515 017 / b3149a58ea13`
│                                     → `1 550 630 / f48bd9a507c9` → `1 589 633 / a3ff5e1ad77a`
│                                     → `1 594 295 / 47e78fa4a57f` → `1 620 020 / d58a4f2d17ae`
│                                     → `1 620 220 / ef62eba8edb3` → 现在这份），
│                                    每一版写进来的时候都是真的。
│                                 ⚠ **别把这里当「远端上一版」的线索** —— 远端现在是什么版本
│                                    **只能现查**（`_verify/deploy-check.js`）。本行以前写过
│                                    「那个 1 467 029 字节的版本现在是远端上一版」，那句话
│                                    早在好几轮之前就不成立了（**没人管的历史断言 = 错的**）。
...
├── _verify\                  ← 验证 harness（19 套 + 探针，见 §7）
├── fonts\
...
| 行号 | 区块 | id / 类 |
|---|---|---|
| 5677 | **手机比例下的入口菜单**（窄屏 / 极扁窗口才出现） | `#mobile-nav` `.mobile-nav` |
| 5696 | 时钟卡片 | `#clock-tz-card` `.clock-container` |
| 5715 | 交互式日历卡片（默认折叠） | `#calendar-card` `.calendar-container.collapsed` |
| 5741 | AI 对话区域 | `#ai-card` `.ai-container` |
| 6013 | SillyTavern 角色卡编写器（折叠入口卡片） | `#st-card` `.st-container` |
| 6053 → 6054 | 编写器全屏浮层 | `#st-overlay` → `#st-shell` |
| 6080 | **🔌 API 全局配置按钮**（在小游戏按钮**上方**） | `#api-global-btn` `.game-entry-btn` |
| 6085 | 小游戏入口按钮 | `#game-entry-btn` |
| 6090 | 贴贴按钮（外链） | `.hug-button` |
| 6095 | 小游戏 Modal | `#gameModal` |
| 6170 | **🔌 API 全局配置 Modal** | `#apiGlobalModal` |

> ⚠ 两个按钮**共用 `.game-entry-btn`** 这个类（就是那条 `margin-top`）——
> 所以 `.game-entry-btn` 这类锚点会指向**第一处**（API 全局配置按钮）。要精确就用 `#` 那个 id。
>
> 小游戏弹窗**里面**的三个锚点（第二十四轮加的，不在上表里 —— 上表只收 body 顶层）：
> `#gameCard` **6096** · `#gameShooterWrap` **6159** · `#gameShooterFrameHost` **6160**。
>
> ⚠⚠ **这一整节的行号在 2026-09-24 整体 +83**（首页内容模块化，往 `<head>` 插了一段
> `.st-mod` 的 CSS）。**别按加法推** —— 那次的位移恰好是 83，但上一次实测过
> 「同一张表里 +20 与 +41 并存」。取真值一律 `grep`，然后跑 `_verify/_audit-anchors.js` 核对。
> ⚠⚠ **2026-09-23（第二十轮：Vertex 列模型 + 完整模式 Key）整张表 +6**
> （`</style>` 从 5574 挪到 **5580**）。这 6 行**全部来自样式块**：把原来逐条补的
> 三条 `[hidden] { display: none }`（`.apig-vertex-box` / `.apig-sa-masked` /
> `.apig-verify-out`）**收敛成一条全局** `[hidden] { display: none !important; }`，
> 连带那段「为什么必须兜底」的注释一起写进去。**正文一行没动** ⇒ 三个弹窗内锚点也是 +6。
> ⚠ 上一轮（第十九轮）是 **+58**、第十八轮是 **+20**、第十七轮是 **+194** ——
> **位移形状每轮都不一样**：这一轮「删 3 条加 1 条」净 **+6**，靠「我插了几行」永远推不出来。
> ⚠ 前几轮的教训照旧：**加在 `<style>` 末尾也不是零位移**（`</style>` 之后的锚点照样整体后移）；
> 产品里那句「放末尾 = 零位移」的注释**是错的**（已改）。
> **一律重跑 `_audit-anchors.js`，再拿 `_fix-anchors.js` 逐行核。**
...
> ⇒ 改完产品**先跑 `_verify/_audit-anchors.js`**（只读），它一次把 §2 / §3 **每一行**都指出来
> （漂了的会报「不是横幅；最近的上一个横幅在 N」，§2 直接给真值）；
> 取真值用 `_verify/_fix-anchors.js`，**别按加法推**。
...
| 109 | 小游戏入口按钮 |
| 400 | 顶部标题 |
| 409 | 毛玻璃卡片基类 `.glass-card` |
| 422 | 时钟容器 |
| 440 / 487 | 时区切换开关 / 时区按钮网格 |
| 552 | 数字时钟 |
| 567 | 日历容器 |
| 804 | **AI 对话区域** |
| 951 | 模型设置子面板 |
| 1027 | 流式 / 非流式开关 |
| 1062 | **SillyTavern 风格 `{{user}}` 宏设置** |
| 1175 | 系统提示词预设 |
| 1224 | **酒馆模式 · 角色卡 / 世界书** |
| 1278 | 可折叠字段 |
| 1429 / 1431 | 来源细类配色（这条系统消息从哪来）/「第一段」单独配色 |
| 1472 | 世界书全局设置 |
| 1532 | 条目本轮会不会插入 |
| 1556 | 提示词预设：槽位列表 + 排序 / 编辑 |
| 1744 | 温度滑块 |
| 1760 | 聊天界面 |
| 1814 | 消息行：气泡 + 重试按钮 |
| 1910 | 贴贴按钮 |
| 1942 | Modal 弹窗 |
| 2025 / 2074 / 2092 / 2102 | 游戏列表 / Canvas / 触屏按键 / 十字键 |
| 2168 | **复古线框战机**（整页游戏：卡片放宽档 `.game-card--wide` + iframe 容器） |
| 2195 | **复古模式覆盖** |
| 2497 | 酒馆模式 · 复古皮肤 |
| 2660 | **SillyTavern 角色卡编写器**（多行大横幅） |
| 2749 / 2859 / 2926 | 全屏浮层 / 左侧选项卡 / 右侧内容 |
| 3256 | 条目 / 脚本卡片 |
| 3333 | MVU 变量框架 |
| 3537 | MVU 状态栏外观编辑器 |
| 4057 | 浮层复古皮肤 |
| 4258 | 编写器里的 AI 助手 |
| 4606 | **Code（Agent）页**（多行大横幅） |
| 5190 | Code 页 · 复古皮肤 |
| 5292 | **AI 聊天窗口「放大」模式**（多行大横幅） |
| 5309 / 5321 / 5377 | 遮罩 / 撑满高度 / 桌面居中 |
| 5405 | **手机比例下的入口菜单**（多行大横幅） |
| 5532 | **复古模式：手机菜单照 Win98 / Win2000**（多行大横幅） |

> 多行大横幅有好几处（2660 / 4606 / 5292 / 5405 / 5532）—— 它们是**多行**写法
> （`/* ====` 换行再写标题），单行横幅的提取脚本扫不到标题那一行，
> 所以 `_audit-anchors.js` 会取**整块**再找名字。
> 找它们要 `grep -n "SillyTavern 角色卡编写器\|Code（Agent）页\|手机比例下的入口菜单\|照 Win98"`。
> ⚠⚠ **这五个数字就是上表那五行**（同几个横幅）—— 但它们在 2026-09-23 之前**整整落后了一轮**
> （写的是 2576 / 4439 / 5125 / 5238 / 5365，比表里少 20）⇒ **`_audit-anchors.js` 只核表里的行，
> 不核这行注释**，所以它悄悄错了没人知道。**改表的时候顺手看一眼这行。**
> ⚠⚠ 2026-09-24 又漂了一次：首页内容模块化往 `<head>` 插了一段 `.st-mod` 的 CSS，
> **3150 之后的行整体 +83**（2660 之前的不动）⇒ 表里那 11 行与这 5 个数字一起改了。
>
> ⚠ **别靠加法算** —— 插入点分散时每段的偏移量都不同。**2026-09-23（第十九轮）实测这张表
> 是「一段位移」**：`336` 之后**整体 +58**，而最上面三行（41 / 79 / 109）**一行没动**
> —— 因为这一轮加的 CSS 全在它们**之后**。⚠ 拿不准就把分段写成 `1:0,120:58` 喂给
> `_fix-anchors.js`，它逐行核「旧值 + 位移」是不是横幅，不是就**大声报 ❌**。
>
> ⚠ **2026-09-23（第二十轮）实测：`132` 之后整张 +6**（最上面三行照旧不动）。
> 这一轮样式块里**只删 3 条加 1 条**（三条逐类的 `[hidden]` 收敛成一条全局的），
> 净 **+6** —— ⚠ **「删 3 加 1」看着像 -2，实际是 +6**（全局那条带了一整段注释）。
> 分段写成 `1:0,132:6`。**又一次印证：位移只能现量。**
>
> ⚠⚠ **第十七轮（API 全局配置）实测：`113` 之后整张 +194。** 这一轮只在 `.game-entry-btn`
> 那条规则后面插了**一处** CSS（约 194 行：全局配置弹窗 + 「配置来源」开关 + 复古覆盖），
> 所以位移是**统一的**。⚠ 但**别把「这次是统一的」当成规律** —— 上一轮就是两种位移并存。
> ⚠ 取真值的可靠办法：`node _verify/_fix-anchors.js`（一次性工具，只读）——
> 它按 `旧值 + 位移` 逐行核「那一行**是不是横幅**」并把横幅原文打出来，
> **每行都能人工核**。⚠⚠ 别用它早期那版「单调对齐 + 共享字」的启发式：**实测会错**
> （本轮就把「系统提示词预设」对到 1047、「条目本轮会不会插入」对到 1438，
> 而真值是 1091 / 1448）—— 两个都**是横幅**，所以光看「是不是横幅」看不出来。
...
| 键 | 内容 | 写入点 |
|---|---|---|
| `aiChatConfig` | 整个 `aiConfig` 对象（供应商 / 密钥 / 提示词 / 酒馆设置 / 宏设置 / 第一段 / **`followGlobal`** / **`own`**） | `aiPersist()`（唯一入口）、`persistUserMacro()`、`persistSystemPrompt()`、`persistTavern()`、`onTavernSetting()` 等 |
| `apiGlobalCfg` | **API 全局配置**（`apiGlobal`：服务商 / Base URL / Key / 模型 / 模型列表缓存 / 温度 / 上限 / 流式 / 第一段）。⚠ **全站唯一真相源** —— 【ai对话】默认跟随它，【编写器】的「跟随」也指向它 | `apiGlobalPersist()`（改它的只有主页那个弹窗） |
...
> ⚠ **本节不写行号**（2026-09-23 第十八轮改的）：这里原来有几处「`let aiConfig` 在 6795 行」
> 这种数字，实测**早就漂了**（真实值 7549），而且没有任何工具管它们。
> 带着错的数字比不写更坏 ⇒ 要定位一律 `grep` 符号名
> （`let aiConfig = ` / `const API_GLOBAL_FIELDS` / `let stAi = ` …），别照抄数字。

### `aiConfig`（默认值 `AI_DEFAULTS`，实例是 `let aiConfig`）
...
### `apiGlobal` / `aiConfig.own`（API 全局配置）

| 对象 | 字段 |
|---|---|
| `apiGlobal`（存 `apiGlobalCfg`） | `provider` / `baseUrl` / `apiKey` / `model` / `models`（拉过的列表缓存）/ `temperature` / `maxTokens` / `stream` / `firstSeg` + **Vertex 四格** `authMode`（`key` / `sa`）/ `project` / `location` / `saJson` —— **全站唯一真相源**，只有主页【🔌 API 全局配置】那个弹窗改它 |
| `aiConfig.followGlobal` | 布尔，默认 `true`。【ai对话】的「配置来源」：`true` = 跟随 `apiGlobal`，`false` = 用 `aiConfig.own` |
| `aiConfig.own` | 【ai对话】切到「独立配置」时自己那一套（字段同 `apiGlobal`，**没有** `models` 缓存）。⚠ 它跟 `apiGlobal` **互不影响**。⚠ 切到独立时是**整份快照**（含 Vertex 那四格），所以独立模式**能**用全局配好的 Vertex 项目 / SA，只是**这个面板改不了**那几格（界面上有一句明说） |

⚠⚠ **`aiConfig` 顶层那 12 个 API 字段是「生效值」，不是真相源。** 全站 40 多个读点
（`aiConfig.apiKey` / `aiConfig.model` …）读的都是它，只在两处刷新：
`initApiGlobal()`（载入时）与 `saveApiGlobal()`（全局面板保存后）—— 都走 `aiApplyEffective()`。
⇒ **别在别的读点改成「按来源取」**：漏一处就是一处静默的旧值。
⚠ **加字段要同时改三处**：`API_GLOBAL_FIELDS`（白名单）/ `API_GLOBAL_DEFAULTS` /
`apiGlobalNormalize()`。漏了白名单 ⇒ `apiGlobalPick` 静默不抄那一格
（症状是「切一次来源那一格就空了」）；漏了归一化 ⇒ 从 localStorage 读回来是脏值。
⚠⚠ **还有第四处：`getAiConfigFromInputs()`**（【ai对话】那个面板）。它返回的是**新对象**，
`aiCommitInputs` 里 `aiConfig = cfg` 会把没列进去的字段**整个抹掉**，接着
`aiSyncOwnFromEffective()` 又拿它去 `apiGlobalPick()` ⇒ 在【ai对话】点一次保存，
独立那份里**没被列进去的字段全被打回默认**。第十八轮就在这里漏了 Vertex 那四格
（画面上没有任何东西会变，只有真去用 Vertex 才发现）。**漏了它比漏前三处更隐蔽。**

⚠ 写 `aiChatConfig` **一律走 `aiPersist()`**（唯一入口）；改 `apiGlobal` 走 `apiGlobalPersist()`。
⚠ 用户**真的在【ai对话】里改了**那几个字段时才调 `aiSyncOwnFromEffective()` 同步 `own`
（它自己带 `followGlobal === false` 的守卫）—— 放进 `aiPersist()` 里无条件跑的话，
跟随模式下改一次系统提示词就会把用户那份独立配置**静默冲掉**。

### `stAi` / `stAiUi`（编写器 AI 助手）

| 对象 | 字段 |
|---|---|
| `stAi`（存 `stAiCfg`） | `mode`（`follow` / `own`）/ `provider` / `proto`（`''` = 跟着服务商）/ `baseUrl` / `apiKey` / `model` / `models` / `temperature` / `maxTokens` / `stream` / `firstSeg`（只在 `own` 下生效，`follow` 时现读 **`apiGlobal.firstSeg`**） |
...
`stAiCfg()` 是**唯一**的取配置入口 —— `mode === 'follow'` 时走 `stAiFollowCfg()`（现读 **`apiGlobal`**），
`own` 时读 `stAi`，返回值统一带上解析好的 `proto` 与 `providerLabel`。

⚠⚠ **`follow` 指向 `apiGlobal`，不是 `aiConfig`。** 第十七轮改的 —— 在那之前它读 `aiConfig`，
而 `aiConfig` 会跟着【ai对话】的「独立配置」一起变，等于编写器**又悄悄跟回了对话**。
`stAiOwnFields()` 里那张「跟随到了什么」的只读表**也必须读 `apiGlobal`**（同一个坑，两处）。
...
N="C:/Users/YanSaki/.workbuddy-ai/binaries/node/versions/22.22.2-3/node.exe"
...
它是留给「单独跑某一层」的时候用的。另外 `G:\saki\_verify\` 那 19 套是**零依赖**的，不用 `NODE_PATH`。）

> ⚠⚠ **这一份驱动第二十一轮才修好，之前它一直是「跑不动」的**（不是套件坏了）：
> - 它原来也用 **`spawnSync`** ⇒ 8 层**全部 `0.0s` + 「没有可解析的收尾行」**、合计
>   「0 通过 / 0 失败」。⚠ **`0.0s` 是唯一线索**（真跑过的层不可能 0.0s）。
>   ⇒ 已改异步 `spawn`；⚠ **本地那份早改过了，外部这份漏了** —— 修的时候要问「这件事有几份实现」。
> - 单层上限原来是 **360 s**（项目规矩是 ≥ 900 s）⇒ 已改成 **900 s**。
> - **视觉层的收尾删 profile**原来是 `kill()` → `rmSync(profile, {maxRetries: 8, retryDelay: 150})`，
>   这一句**永不返回**。⚠⚠ 真因是**量的**（猜错两次之后）：
>   **这个 tmpdir 里删一个文件要 ~200 ms**（写只要 6 ms，实测 216 ms vs 6 ms），
>   而一个 Chrome profile 有几百个文件 ⇒ 光删就 100 s 起。
>   ⚠ **跟 Chrome、跟重试次数、跟产品都无关**（把 `maxRetries` 从 8 调到 0 照样卡）。
>   ⚠ 换成 `fs.promises.rm` + 3 秒上限**也不行** —— `process.exit()` **拦不住它**：
>   Node 退出时会等 libuv 线程池里那个正在跑的删除任务做完
>   （实测：汇总行打完 `371/371 通过` 之后进程还活着 30 s+，驱动那边只看到 `TIMEOUT`）。
>   ⚠ 症状：**检查在 108 s 就全跑完了**，然后卡到超时，报 `TIMEOUT`（看着像「这一层跑不完」）。
>   ⇒ 已改成**派一个脱离的子进程去删**（`detached: true` + `unref()`）：
>   父进程立刻能退，子进程自己慢慢删；删不掉也无所谓，`_verify/_purge-tmp.js` 会收。
>   **「收尾干净」和「进程能退」只能选一个 —— 选能退。**
> - ⚠ 驱动现在会**把没通过的层/套件打印尾部输出**（最后 25 行，只打印不落盘）——
>   上面这几条**都是靠它 + 「收尾打时间戳」才看见的**；在那之前只有一行 `0.0s` / `TIMEOUT`。
> - ⚠⚠ **本地那 19 套也各删一次自己的 profile**（`rmSync` + `maxRetries: 8`）。按上面这个
>   ~200 ms/文件的速率，**每套的收尾本身就是几十秒的开销**。它们现在**没有**卡
>   （每套 ~285 s 能跑完），所以**本轮没动**（17 个文件、风险大于收益）——
>   真被咬到时，照上面那个「派后台子进程删」的写法改一行就行。

### jsdom 层（7 个）

| 脚本 | 覆盖 | 断言数 |
|---|---|---|
| `static_check.js` | 静态：id 唯一（**244**）· `getElementById` 字面量引用存在（**169**）· 内联 handler 都有定义（**204**）· 标签配平（HTML 骨架 5 种 + JS 生成物里的 `<details>`）· 大括号配平（script + style）· **style 块提取没错位**（先摘 script 再找 style，见下）· 若干关键 CSS 规则存在 · **`ST_`/`st` 前缀标识符都声明过（890）** | **17 项** |
...
| `verify_steditor.js` | 角色卡编写器（草稿模型、序列化、PNG、导入导出、酒馆助手、MVU） | 522 |
...
> | 2026-09-24 实测（第 21 轮后） | **244** | **169** | **204** | **890** |
>
> ⚠ 第 21 轮这一行跨度很大（+89 / +64 / +31 / +78），因为它**积了 12 轮**
> （第 10~21 轮：世界书追加 / API 全局配置 / 服务商扩充 / Vertex 两轮 / 状态栏解析与摆放 /
> 编写器首页 …）—— 中间那些轮**都没回来重取过**这几个数。
> 这恰好印证了上面那句：**「检查是推导的」不等于「文档里抄的那个数是对的」**。
>
...
⚠ 也**不写死期望总数**（理由同上）。
⚠ **本地那 19 套不需要这第四条**：它们的正则 `N 通过 / M 失败` 要求两边都是数字，
...
>
> ⚠ 这个目录的收尾格式跟 `_verify/` 那 19 套**又不一样**：这里是 `166/166 通过`
...
### 编写器专项：`G:\saki\_verify\`（19 个，共 2 709 条）
...
| `st-ai.js` | **编写器的 AI 调用**：配置两种来源（**跟随的是 `apiGlobal`，不是【ai对话】** —— 两边**故意设成不同的值**才分辨得出读的是哪一份）、双协议请求形状、四种模型列表形状、两套 SSE 拼接、JSON 抠取边界、批量扩写 / **单条改写** / 开场白落卡、**「第一段」与提示词结构预览**、Key 不进卡、失败路径；**空列表要拒绝**（第二十一轮修：⚠⚠ 这条原来断言的是 `/没返回任何模型/` —— **旧文案**。产品在第十四段把 `stAiFetchModels` 拆出 `stAiFetchModelsOnce` 时顺手把报错改成了「返回里没有模型（字段名对不上？）」，**断言没跟着改** ⇒ 套件一直红、红到第二十一轮整跑才被翻出来。⚠ 旧串在产品里**只剩一处注释**，所以 `grep` 产品会骗人 —— 只能靠「跑一遍看它红」；⚠ 现在拆成两条：① **拒绝**（不静默返回空数组）② 报的错说的是「**没有模型**」这件事。判据故意**不写死整句**，否则下次纯文案微调又要来改，那种「永远在报的检查」最后会被无视） | 344 |
| `apig-verify.js` | **API 全局配置**（第十七轮加：主页【🔌 API 全局配置】弹窗 + 三处「跟随」的指向）：按钮在**小游戏按钮上方**（认 id + `compareDocumentPosition`，不靠 `indexOf` 的下标）、**走真按钮**开弹窗 + **命中测试**（屏幕正中点到弹窗自己）、弹窗字段齐全、填值 → 点**真保存按钮** ⇒ 内存 / `apiGlobalCfg` / 刷新往返都对、**全局配置里不该有 `systemPrompt`**（那不是 API 配置）；**一次性迁移**（老 `aiChatConfig` ⇒ 全局 + `own` 都播种、`followGlobal` 默认 `true`、生效值 == 老配置 ⇒ 外观不变、**已经落盘所以不会二次播种**）+ **二次载入不许把用户改过的全局值盖回去**；【ai对话】的来源开关（跟随时那批 API 控件是**真 `disabled`**（6 个全查）+ 第一段也锁 + 给了「打开全局配置」的出口、切独立 ⇒ 解锁、独立模式下改 Key **走真保存按钮** ⇒ `own` 记下而 `apiGlobal` **一点没动**、切回跟随 ⇒ 生效值回到全局**而 `own` 还留着**、**跟随模式下改系统提示词也不许冲掉 `own`**）；**编写器跟随的是全局**（⚠⚠ 套件里把「对话那套」**故意改成第三组值**，再断言编写器读到的仍是全局 —— 两边写一样的话这条断言等于没写）；**「第一段」两边都从全局取**（⚠ 这句说的是**两边都在跟随模式**时：【ai对话】切到独立之后，**编写器仍在跟随模式**、照样读全局 —— 两者各有各的 `follow` / `own` 开关，**对话切独立不会把编写器拖走**；编写器自己的独立模式取 `stAi.firstSeg`）；**空响应要说得出原因**（第十九轮加：⚠⚠ 这条打的是**接线**不是函数返回值 —— 桩返回一个空响应，走**真请求路径**看 `nonStreamChat` / `stAiChat` 会不会把 `promptFeedback.blockReason` / `content_filter` 抛出来；⚠ **两份实现各测一遍**（【ai对话】一份、编写器一份），只改一处会被抓到；⚠ 配对照组「`finishReason = STOP` 的真空白不算错」—— 没有它的话「见空就抛」也能把上面几条骗绿；⚠ 协议要和桩的**响应形状**配套，喂 OpenAI 形状却把协议设成 gemini 是**测试自己搭错了对象**）；弹窗失败路径（测试连接的网络不通 / 401、获取模型**去重**、三种返回形状、剥 `models/` 前缀、空列表要报错）；**复古皮肤**（先取非复古对照组：不是 Win98 灰、有圆角 ⇒ 加 `retro-mode` 后按钮 / 卡片变 `#c0c0c0`、圆角归零、说明块变白 ⇒ 退出后复原；⚠ 第十九轮把 **SA 掩码条**与**验证结果区**也纳进来 —— 新组件忘了补复古覆盖就会出现「白底黑框的页面里插了一块浅绿毛玻璃」，而那两条是**先把控件显出来再量**的，不是量一个 `display:none` 里的东西）；**【ai对话】的模型框能手输**（INPUT + datalist、手输一个候选里没有的名字也能存进 `own`）+ **独立模式 + Vertex 时那句交代要出现**（跟随时收起）+ **保存不许把 Vertex 四格打回默认**（`getAiConfigFromInputs` 漏带 = 画面上看不出来）；**完整模式下 API Key 那一格必须真的藏住**（第二十轮加：⚠⚠ 这一条是**用户报的 bug** 的回归测试 —— `keyF.hidden = true` 一直在设，但 `#apig-key-field` 是 `.ai-field`，而 `.ai-field { display: flex }` **盖掉了 `hidden` 自带的 `display: none`** ⇒ 属性设对了、**画面上照样看得见**；所以断言读 **computed `display`** 而不是读 `hidden` 属性，配「快速模式 ⇒ 真的显示」作对照；⚠ 同类：`.apig-vertex-box` / `.apig-sa-masked` / `.apig-verify-out` 也都写了 `[hidden]` 兜底，第二十轮把这三条**收敛成一条全局** `[hidden] { display: none !important; }`）；**「获取模型列表」不许要求先有模型名**（第二十轮加：⚠⚠ 也是用户报的 bug —— `apigReadyCheck` 的 Vertex 分支要求 `model` 非空，而 `apigFetchModels` 调它时**没带 `needModel:false`** ⇒ Vertex 下必须先手打一个模型名才能点「获取」，把「获取」这个功能本身废掉了；断言「模型名空着 + 快速模式 Key 齐 ⇒ `apigReadyCheck(cfg, {needModel:false})` 通过」+ 对照组「要模型时照样拦」+ **走真按钮**点一下确认它真的发出请求（不是只测那个函数）+ 列不出来时**退回内置候选且把失败原因说出来**）；收尾零报错 | 135 |
| `vertex-verify.js` | **服务商大扩充 + Vertex AI + 手输模型名**（第十八轮加）：**服务商表**（37 项、`ST_AI_PROVIDERS === AI_PROVIDERS` **同一张表**、分组顺序、`custom` 必须在末位（`stAiProviderOf` 的兜底）、**只有 `custom` 允许不写死 `proto`**（它是「按 URL 猜」）+ 对照「`proto` 这个**键**每项都得有」）；**协议判定**（⚠⚠ 一定要打在 **resolver `stAiProtoOf`** 上，不能只打 `stAiGuessProto` —— 「猜」和「决议」是两步，第十八轮的真 bug 就藏在缝里：预设表给 `custom` 写了 `proto:'openai'` ⇒ resolver 整条跳过 URL ⇒ 粘 Anthropic 域名也按 OpenAI 形状发，而只测「猜」的断言**照样全绿**；另配「空 Base URL」三条，其中只有 `gemini-native` 是真的「只有表能答」—— 另两个名字本身就是关键词、猜也猜得对）；**半成品 cfg 兜底**（`stAiEnsureProto`：全局面板 `apigGather()` 给的对象**没有 `proto`** ⇒ 少了这步 gemini/vertex 会按 openai 发出去）；**gemini URL 三条路**（AI Studio 原生 / Vertex 快速模式**不带** `projects/` / 完整模式带 `projects/{p}/locations/{l}`、`location=global` 时端点**不带地区前缀**、流式必须 `alt=sse`、列模型两条路）；**请求体**（system 抽进 `systemInstruction`、`assistant → model`、contents **非空**且**首条必须是 user**、`generationConfig`）；**Service Account**（坏 JSON / 缺字段 / 完整 JSON 解析、`token_uri` 兜底、**假私钥必须抛错而且要说清是「私钥」读不出来** —— 只查「抛没抛」的话，把 `importKey` 的错误吞掉、后面 `subtle.sign(key=null)` 照样抛，用户看到的是「parameter 2 is not of type CryptoKey」这句天书）；**弹窗 UI**（非 Vertex 时那几格 `hidden` 且 computed `display` 真的是 `none`（`display:flex` 会盖掉 `hidden` 自带的 `none`）、切 Vertex ⇒ 显示、默认快速模式、Base URL 自动填、快速 / 完整两种模式各自的显隐、**模型是 INPUT + datalist 能手输**、服务商下拉有分组）；**`apigReadyCheck` 两种模式各自的必填项**（快速要 Key、完整要 SA JSON + project、都要模型名；⚠⚠ 第十九轮修：**项目 ID 不再要求手填** —— 从 SA JSON 的 `project_id` 派生（酒馆的 `getProjectIdFromServiceAccount()`），两处都没有才拦，而且**要明说是缺项目** —— 否则拼出 `/v1/projects//locations/…` 拿个 404 回来，报错里看不出是「少了项目」）；**`stAiReady` 的 Vertex 分支**（⚠⚠ 第十九轮**补的覆盖缺口**：全局面板那份 `apigReadyCheck` 被测了，编写器 /【ai对话】用的 `stAiReady` **一条断言都没有** —— 而这一轮最硬的 bug 恰好在它里面：完整模式 `apiKey` **本来就是空的**（鉴权走 SA 换的 token），通用那条「没填 Key」检查会把它拦下 ⇒ **完整模式永远跑不起来**，而 105 条全绿的套件里**完全隐身**）；**`safetySettings`**（⚠ 不发它 Gemini 就按**默认策略**拦、角色扮演很容易撞上，而表现是 `candidates` 根本不存在 ⇒ 前端拿到空串、看着像「AI 没说话」；AI Studio 5 条 + Vertex 另 5 条（含 `JAILBREAK`）、阈值全是 `OFF`（照抄酒馆，不是 `BLOCK_NONE`）；⚠ 取元素一律 `(x || [])` —— 注入把整个字段删掉时 `undefined.every` 会抛、套件当场炸掉连汇总行都没有，反向测试那一针就白跑了）；**`stAiWhyEmpty` 的分派**（三种协议字段名不同：gemini `promptFeedback.blockReason` / `finishReason`、anthropic `stop_reason`、openai `finish_reason`；⚠ 配对照「正常 STOP 不算错」）；**SA JSON 掩码态**（存过之后只显示「存的是哪一份」（`client_email · project_id`）、textarea `hidden`；⚠⚠ 掩码态下 `apigGather()` **必须仍拿得到那份 JSON** —— 掩码只是藏起来、value 一直留着，改成「掩码时清空」的话用户打开面板看一眼再点保存密钥就**静默**没了）；**「验证 JSON」分步**（① JSON 语法 → ② 必填字段 → ③ 私钥导入 WebCrypto → ④ 签 JWT（**前四步纯本地、不发一个网络请求**）→ ⑤ 换 access token → ⑥ 列模型；⚠ 每步失败都要说清是哪一步，尤其要把「被 CORS 挡了」和「凭证不对」分开；套件用假私钥验证它**停在 ③ 且不说 ⑤**，证明它没乱发请求）；**主页【ai对话】也走共用协议层**（`aiEffectiveCfg` 认出 gemini）；**README 里「服务商表（N 项」跟表里实际条数必须相等**（这句数字没人管，本轮就漂过：写 38、实际 37）；**列模型是「一串候选」而不是一条 URL**（第二十轮加：⚠⚠ 也是用户报的 bug —— 原来只有一条 `…/v1/projects/{p}/locations/{l}/publishers/google/models`，而 **v1 那条资源根本没有 `list` 方法**（只有 computeTokens / countTokens / embedContent / generateContent / predict / rawPredict / stream*）⇒ 注定 404；`publishers.models.list` 在 **v1beta1**。现在按顺序试：① `v1beta1/publishers/google/models`（**不需要 project**，也是唯一确认有 `list` 的）→ ② `v1beta1/projects/{p}/locations/{l}/publishers/google/models` → ③ `v1/…`（老形状兜底），断言三条的顺序与形状 + 「前两条都是 v1beta1」）；**`stAiModelIds` 要认 `publisherModels` 字段**（第二十轮加：⚠ 就算 URL 修对了，解析层不认这个字段也会得到**空列表** ⇒ 报「这个接口没返回任何模型」，看着像 URL 错、其实是解析错 —— 与 `data` / `models` / 裸数组三种形状并列测）；收尾零报错 | 147 |
| `persona-verify.js` | **人设生成器**（写一句设定 / 要求，AI 按一份模板逐项填好，生成一整份人设）：选项卡在不在 + **紧跟在 `code` 后面**（配一条反向对照：它**不再**挨着「角色描述」）+ **走真按钮**能切过去、面板元素齐全（含**反向对照**：还没生成时结果框与写入 / 追加 / 复制三个按钮**不该在**，空态要说话）、**默认模板逐字**等于用户给的那份（测试里**独立写一遍**，不引用产品常量 —— 拿常量比自己永远相等）+ **项数用另一套写法独立算一遍**（模板里半角 / 全角冒号**混着用**：全角那 9 项要认、`- 风格：` 要认成「风格」而不是「- 风格」、三个重复子项只算一项）、模板可改且**存进 localStorage**、**刷新还在**、**清空 = 回到默认模板（而且是删键不是存空串）**、超长截断、「恢复默认模板」按钮、**面板参数活在状态里**（走**真实 `oninput` 路径** + 重绘之后还在）、提示词里带上**用户原文 + 模板全文 + 这张卡现在的名字**、未配 AI 就不发请求、围栏清洗（**没包围栏的原样留着**作对照，洗多了会吃掉真正的第一行而看不出来）、**「模板 N 项里对上了 M 项」**（漏项 → `warn` 且说出漏了几项，全中 → `ok`）、**写入 / 追加**（空描述直接写、非空描述**必须弹 confirm**，答「不要」卡一点不动 / 答「要」才整个替换、追加不弹窗、描述是纯空白时追加不留空行、没有结果时只给提示不动卡）、复制、**忙态扛得过重绘**（重绘之后按钮还是「⏳ 生成中…」）、失败路径（网络不通 / 401 / 空回复 / 模型回寒暄），收尾零报错；**「模板」默认折叠**（收起时量**外层网格容器**自己 + 做**命中测试**，展开时把同样两样再量一遍 —— ⚠ 因为 `grid-template-rows: 0fr` 的**裁切不影响 `getBoundingClientRect()`**，里面 textarea 的 rect 照样满高，**拿 rect 当判据的话收起 / 展开量出来一模一样**，这条也留成对照断言记着）、**配置行排在模板折叠块下面**（对照组盯的是「两个节点都在顺序表里找到了」，防 `indexOf` 找不到时返回 `-1` 让主断言**天然成立**）、折叠态**从状态渲染**（重绘之后还在，不是只活在 DOM 上）、**生成器的「工作状态」贴在「✨ 生成人设」按钮上方**（第十三轮加：DOM 顺序上排在前 + 默认没字且 `display:none` + `scope='persona'` 时进页内格而**常驻条同时留空**（一条消息只出现一处，两边都写用户会看到两遍）+ 切走 ⇒ 退回常驻条**消息不丢** + 切回 ⇒ 又接住；⚠ 同样配「两个节点都在顺序表里」作对照防 `indexOf` 返回 `-1`）+ **生成结果追加到世界书**（第十四轮加，第十五轮改取名来源：结果区多一个「📖 追加到世界书」，**走真按钮**点一下 ⇒ 卡里的 `bookEntries` 真的多一条、`content` 就是生成结果、默认启用，而 `comment` / `keys` **都取生成结果里「姓名:」后面的字符**；⚠⚠ 套件里**故意**让「姓名:」（艾莉丝）跟卡名（夜乃）**不同** —— 一样的话「取的是 `姓名:`」和「取的是卡名」读出来是同一个字符串、断言根本分不出从哪儿取的；取值函数本身另测半角 / 全角 / 行首装饰 / 整行加粗 / **CRLF**（JS 的 `.` 不匹配 `\r`，不归一化会整行匹配不上）/ 取不到那三种；还有**切到世界书页真的多渲染一条**（不能只活在状态里）；配对照：**原有条目一条都不许动**（备注 / 正文 / 关键词）、**没有结果时点它条目数不变**、**加了新按钮不许挤掉老的那三个**、**没有「姓名:」时回落卡名且提示说明「用的是卡名」**、**两边都没有 ⇒ `keys` 为空 + 备注回落成「人设」+ 提示是 `warn`**）+ **「要不要拿卡名当人物姓名」的勾选框**（第十四轮加：默认勾上 ⇒ 提示词里有「这张卡现在的名字」；**走真勾选框**点掉 ⇒ 状态跟着关、提示词里不再有卡名、重绘之后仍然是关的、**真实请求**里也不带卡名（直接调 `stPersonaUserPrompt` 只证明那个函数写得对，证明不了请求真的走了它）；配对照：关掉的只是**名字**，用户的要求与模板照样在）+ **四个按钮的反馈 +「已经添加过了」三选一弹窗**（第十六轮加：四条提示都带人物名（取「姓名:」、取不到回落卡名；两处都没有就**整段省掉**、不印空引号）；**走真按钮**连点两次 ⇒ 第二次弹三选一，标题写「「艾莉丝」已经添加过了，请选择」；⚠ 弹窗期间**条数 / 备注一样都不许动**（「问」不等于「写」）+ **命中测试**（屏幕正中必须点到弹窗自己 —— ⚠ 量 rect 是量不出「被别的层盖住」的）；**取消** ⇒ 什么都不做（条数不变、原条目正文不动、记账也不动 ⇒ 再点还问）；**追加为新条目** ⇒ 条数 +1、id 与原来那条不同；**覆盖** ⇒ **条数不变、所有 id 都不变、原位改写**，配对照：用户在世界书页改过的启用 / 正则开关**不许被抹掉**；**重新生成一份人设 ⇒ 记账清零**，于是「第一次点直接追加、第二次点才问」；**记账里那条被删了** ⇒ 当新条目处理、而且提示要**说出来**）| 277 |
| `st-code.js` | **Code（Agent）页**：工具调用协议（两套）、19 个内置工具 + **工具型 skill 注册成工具**的行为与边界、**工具型 skill 的权限白名单**（三层能力、`fetch_url` 永远不给、次数上限、没给 `value` 不许当空串写）、**`fetch_url` 的地址闸门**、Agent 循环与消息序列、skill 四条导入路径（目录 / 选文件 / **从链接** / **拖拽**）+ **触发词自动注入正文**、**`set_tasks` 任务清单**、**项目记忆与 `remember`**、**`read_doc` / `write_doc` 的状态栏与 MVU 往返**、**`read_regex` / `write_regex` / `read_th` / `write_th` 按条目改 + MVU 自己的东西挡住**、**截断标记闸门与 `read_doc` 分段读**、**skill 落盘（IndexedDB + 降级 + 迁移）**、**工具型 skill 的沙盒**（摸不到卡 / 写不了 localStorage / 上不了网 / 超时掐得掉）、**skill 自报进度**（单向通道、没申请就被丢、节流 + 次数上限、返回后收走）、会话历史落盘、窄屏单栏切换、**「第一段」开关**（默认不带 / 打开后必须排在最前 / 空值不发 / 落盘往返）、JSON 只读不写、**记忆输入框写超了当场看得见**（`maxlength` + 计数器变红 + 折叠摘要带字数 —— 不静默丢字）、**Gemini / Vertex 协议下明说「工具调用不支持」**（报的错点名协议 + **一个请求都没发** + 给了两条出路 + 没卡在 `running`；对照 = 同一步换成 openai 协议**就会**发） | 676 |
| `home-verify.js` | **编写器「首页」选项卡**（第二十一轮加：概览 / 新建 / 导入 / 从别处搬条目）：选项卡**排在最前** + **默认落点**就是它 + **走真按钮**切走再切回；面板元素齐全（含**反向对照**：还没选文件时勾选清单**不该在**）；**概览数字跟着真实数据走**（改卡名 ⇒ 正文字段 `0/8 → 1/8`、加条目 ⇒ 世界书 `0 → 2` 条，**每条都配「真的变了」的对照** —— 少了它「数字写死了」也能全绿）；**两种来源形状都要认**（角色卡 V2 的 `data.character_book` + 独立世界书的 `entries` 是**对象**而非数组）+ 没有 `entries` 时归空且给得出话；导入是**追加**不是替换（原有条目一条不少、内容不变、id 不换）+ **不与来源共享对象**（⚠⚠ 这条**必须先抓住来源对象的引用再导入**（`window.__srcRef`）—— 原来写成「导入后重新 feed 一次再读 `extractSrc.entries[0]`」，而 `parseWorldBook` 会 parse 出**全新对象**，比的是两个不同对象 ⇒ **永真断言**，反向测试 R5 抓出来的）+ 导入后清场；勾选真的管用（全不选 ⇒ 按钮 `disabled` 且一条不进、只勾一条 ⇒ 只进一条）；⚠⚠ **模式不残留**（模拟「点了提取条目又在文件框点取消」—— `change` 根本不触发，模式靠下一次 `stImportPick()` 覆盖掉；另配 `mode=card` 端到端喂卡，确认走的是「换整张卡」而不是弹条目清单）；**首页内容模块化**（第二十一轮同日加：首页不再是「一长条拼起来的字符串」，而是 `ST_HOME_MODULES` 注册表驱动的四个独立模块 —— 断言**顺序按注册表**、**没头像时头像模块整个不渲染**（配「有头像时它出现且排最前」作正反一对）、每个模块都有头 / 箭头 / 内边距层、默认**全部展开**；折叠**走真实点击**，判据是 **computed `grid-template-rows` + 命中测试**（⚠ 收起后 rect 量不出内容，只能问「那个点现在是谁的」）；**收起状态扛得过 `stRerender()`**（状态挂 DOM 上的话会自己弹开）**而且走 `stLoadDraft()` 读得回来**（只读一眼 localStorage 不算数）；⚠ 拆完还要盯住两个老钩子 —— `.st-kv` 还在、整个首页的 `.st-actions` 按钮**合计仍是 4**（外面 `verify_steditor.js` 的 D1c 正是数它）） | 78 |
| `sb-keep-compare.js` | **渲染级证据**：原稿 vs 产物逐元素比对计算样式 + 500×1100 区域 PNG 逐字节 | 32 |
| `smoke.js` | 全应用冒烟：16 个选项卡、装 MVU → 建变量 → 套模板 → 写进正则一条龙、**两处「展开键」**（全屏浮层顶栏 `≡ 选项卡` + **主页卡片的箭头**：箭头存在 / 有 transform 过渡 / 与参照物同时长 / 折叠态转 180° / 重画后仍在 + 两个展开态对照组）、**手机比例下的入口菜单**（切到 390×844 视口：两卡收起 + 时钟/日历作对照仍绿 / 点按钮展开 / 点「AI 对话」直接全屏 / **全屏时没被 `display:none` 压掉** / 切回桌面三件事复原）、**手机菜单的复古皮肤**（三个非复古对照组：按钮不是 Win98 灰 / 有圆角 / 菜单项不透明 → 加 `retro-mode` 后：按钮 `#c0c0c0` + 圆角归零 + 凸起立体边框、卡名标签 Win98 灰 + 凹陷边框、菜单框 Win98 灰 + 凸起、菜单项自己不带底色 + 圆角归零 → 鼠标移上去是海军蓝底白字 → 移开恢复 → 退出复古后按钮复原）、**「正在编哪张卡」标注**（跟编写器卡片的「当前草稿」是同一份数据：改 `card.name` 标注跟着变 + 改回原名作对照 + **在面板里** + **在「角色卡编写器」那一项下面** + **菜单收起后命中测试摸不到它**（配「展开时摸得到」作对照）+ 两个菜单项等宽） | 91 |

```bash
cd /g/saki/_verify
N="C:/Users/YanSaki/.workbuddy-ai/binaries/node/versions/22.22.2-3/node.exe"
"$N" run-all.js                # 全部 19 套：逐套打印 + 合计 + 清单对账
...
> （`smoke.js` 91 / `st-code.js` 676 / 视觉层 371 …，对不上就是少走了）。
>
> ⚠⚠ **而「对得上」也不等于没问题** —— 数字有两种：**写死的**（改了就红，会来收账）和
> **推导出来的**（跟着产品走，**永远不会红**）。`smoke.js` 就是后者：它有一个
> `for (const t of tabs) check('选项卡 ' + t + ' 渲染', …)` —— **每条选项卡一次断言**，
> 所以第二十一轮加了「首页」之后，它是 **90 → 91**，而**没有任何一行代码需要改**。
> 这类数字只能靠**拿实测跟表对**发现（`run-all.js` 打出来的那个数），
> 光看代码是看不出来的。反过来，`st-ai.js` 里那句写死的 `选项卡数量 16` 就会自己红。
> （⚠ 这里原来写的是「`st-ai.js:797`」—— **行号是写死的，加几行注释就漂了**（现在在 810）。
> 文档里的行号跟代码里的数字是同一类东西，能不带就不带。）

> ⚠⚠ **还有一类「跑不了」是 harness 自己的结构缺陷，跟产品无关** —— 第二十一轮整跑实测到两处，
> 都在 `vertex-verify.js` 里，而**它是全 `_verify/` 里唯一一套两样都占的**：
>
> 1. **CDP 端口写死**（`const cdpPort = 9711;`）。其余 16 套都走「先真绑一下、绑得上才算数」；
>    连跑整套时上一轮 Chrome 还没退干净就会占着这个号，症状是「连不上 CDP」。
> 2. **盲等 `await sleep(2500)`** 而不是等 `Page.loadEventFired`。整套连跑时（前面已经起了
>    十几轮 Chrome、主页还要 `@import` 拉 Google Fonts）2500 ms **不够**，页面还没跑完内联脚本
>    ⇒ 紧接着的 `ev('AI_PROVIDERS…')` 抛 `ReferenceError` ⇒ `ev()` 直接 throw。
>
> ⚠⚠ **它的症状极具误导性：`1 通过 / 1 失败`** —— 而那唯一一条**通过**的恰恰是
> `加载后没有未捕获异常`，因为**什么都还没跑**，自然零异常。汇总行完全看不出是「加载没完成」。
> 单跑一次是 **147 / 0**，所以它**不是产品回归**。
> ⚠ 两处都改掉了（端口改成「先真绑一下」，加载改成等真实事件 + `document.fonts.ready`）。
...
这个目录里除了上面 19 套，还有几类**不是回归测试**的文件，别被名字绕进去：
...
| `run-all.js` | **跑全部 19 套的 runner**，带「清单对账 + 汇总行解析 + 合计」。⚠ 它自己也在清单里（`NOT_A_SUITE`），否则会被自己对账拦下。⚠⚠ **必须用异步 `spawn`** —— 这个环境里 `spawnSync` / `execFileSync` / `execSync` **一律返回 `EBUSY`**（连 `where.exe` / `git` 也一样），只有异步 `spawn` 正常。用同步 API 的话那 19 次调用全失败，打出来是「**0 通过 / 0 失败** + 每套一行 `[无] 0.0s ← 没有可解析的汇总行`」—— ⚠ **那个输出长得像「套件全坏了」**，而坏的是 runner 起不了子进程（`0.0s` 是唯一线索：真跑过的套件不可能 0.0s）。见 `RULES.md` 六之四十六。⚠⚠ 2026-09-24 补：**没通过的套件会把最后 25 行输出打出来** —— 套件自己崩掉时那句 `💥 套件自身出错：<stack>` **只存在于完整输出里**，汇总行只打 `1 通过 / 1 失败`（`vertex-verify.js` 实测），原来只能再花五分钟重跑单套才看得到崩点。⚠ 故意**不落盘成文件**：生成物用完就得删，留两份就一定有一份是错的 —— 需要的只是「为什么红」那一小段，打印出来就够了 |
| `suites.js` | **套件清单 + 汇总行正则的单一真相源**（第二十轮抽出来的）：`SUITES` / `NOT_A_SUITE` / `isScratch` / `summariesIn` / `auditManifest` 全在这里，`run-all.js` 与 `run-all.sh` **`require` 同一份**。⚠ 抄一份必然分叉 —— 「两种汇总格式」那个坑本身就是抄出来的（`grep` 只匹配一种 ⇒ 静默少 4 套 / 272 条） |
| `run-all.sh` | **备用驱动**：bash 起子进程，跑完把输出交给 `suites.js --report` 呈现。⚠ 平时**不用它**（`run-all.js` 是首选）；留着是因为它的子进程不经过 node 的 `child_process`，万一哪天连异步 `spawn` 也出问题，它还能跑。⚠ 逐套**串行**，别并发（撞 profile / 端口时红绿都不可信） |
...
| `api-push.js` | **git-over-HTTPS 被代理挡死时的兜底推送**（⚠ **默认 dry-run**，`--go` 才真推；已登记进 `NOT_A_SUITE`）。手法是**用 Git Data API 复刻一模一样的 commit 元数据**（tree / parent / author / committer / message 全从本地 `git cat-file commit` 原样取出）⇒ 远端算出来的 sha **等于本地 sha** ⇒ **零分叉**。每一步都拿本地 `git rev-parse` 的 sha 当**闸门**，任何一步对不上就退出（此时远端 ref 一个字节没动）；最后一步 `force: false`。⚠ 为什么需要它：本机 git 必须走本地代理（直连 `github.com:443` 不通），而代理会周期性返回 `CONNECT tunnel failed, response 502`（实测连续 11 次 / 4 分钟全败），同时 `api.github.com` 直连正常。⚠ 判据：先 `env -u https_proxy -u HTTPS_PROXY -u http_proxy -u HTTP_PROXY git push` —— 报 `Could not connect to github.com:443` 说明代理是**必须**的，绕不开 |
...
| `_audit-sections.js` | **抓「节号撞号」**（只读）：同一文件、**同一个父节内**、同一层、同一号出现两次 ⇒ ❌ 并**退出码 1**；「跳号 / 倒序」只打软提示（**不计入失败** —— 顺序异常可能是故意的，`MEMORY.md` 就把 ⑧ 排最前）。⚠⚠ **判据范围是「同一父节内」，不是「同一文件内」** —— 第一版按同文件比，日报 / CHANGELOG 里每个 `## 第 N 段` 各自的 `### ①…⑧` 全被判成撞号，**一个文件报 27 处假撞号、全库 24 处**（假红比不检查更坏：它会让人开始无视这个工具）。认**四种**节号写法：中文数字 `一、`、阿拉伯 `1. `、带圈 `①`、子节 `六之十九`；⚠ `②–⑦` 这种**区间**要显式排除，否则立刻产生假撞号。用法：`node _audit-sections.js [文件名…] [--quiet]` |
| `_selftest-sections.js` | `_audit-sections.js` 的**反向测试**（自包含：备份 → 注入 → 断言 → 还原 → 核 sha1，**14 项**）。它要证明**四件事**：① 基线不注入 ⇒ 绿；② 注入**真撞号** ⇒ **必须红**（否则它是个摆设）；③ 注入**跨父节的同号重复** ⇒ **不能红**（给上面那个范围 bug 立的碑）；④ 注入**跳号** ⇒ 只打 ⚠ 软提示、**退出码仍为 0**（钉住「软提示不计入失败」这条行为断言）。⚠ 没有它，「0 撞号」只能说明**这个检查从来没抓到过东西**。⚠ 收尾那句统计是**数出来的**不是写死的 —— 写死「三项」在加第四项时就过期了 |
...
| `_fix-anchors.js` | **`_audit-anchors.js` 的取真值搭档**（`_` 前缀，只读）：审计只说「哪几行对不上」，它负责**把真值算出来**。§2 走审计那套判据直接取值；§3 走**可验证的位移**，逐行核「旧值 + 位移」那一行**是不是横幅**（`/*` 开头），不是就打 ❌ 并列出来。⚠ **位移支持多段**：一轮里插了两处 CSS 就是两段，写法 `node _fix-anchors.js 138:13,220:27`（`<旧行号起>:<到这段为止的累计增量>`，升序、增量必须递增；不传参数退回单段 `113:194`）。⚠ 那个「旧行号起」= 插入块在**去掉本轮插入之后**的行号，**别拿新行号当旧行号用**。⚠⚠ 位移**每轮都不一样，而且不等于「我插了多少行」** —— 第十八轮插了两处共 27 行，实测净增只有 **20**（插的同时也删了几行）；拿不准就把位移传错 —— 它会**大声报 ❌**，不会静默给错值。⚠ 早先那版「单调对齐 + 共享字」的启发式**实测会错**（一次错 2 行）⇒ 已弃用。⚠ 它输出里每行都带横幅原文，**要人核一眼**再往 §2 / §3 里抄 |
| `_lint-suites.js` | **改完套件先跑这个**（`_` 前缀）：把 `_verify/*.js` 逐个过 `node --check`，语法不通就指名道姓报出来。⚠ 为什么值得单开：套件里到处是 `ev(\`…\`)` 这种**模板字符串**，注释里只要出现**一个反引号**（比如写「\`custom\` 那条」），字符串就提前结束，报的是 `SyntaxError: missing ) after argument list` —— **指向的行号是模板字符串开头**，看着跟那句注释毫无关系，人眼扫不出来。⚠ 第十八轮**连踩三次**（`vertex-verify.js` 两次、`apig-verify.js` 一次），所以做成闸门。⚠⚠ **它也必须用异步 `spawn`** —— 用 `spawnSync` 的话会**全红假报**：`❌ 60/60 个文件语法不过`，看着像全仓炸了，其实只是起不了子进程（第二十轮实测，见 `RULES.md` 六之四十六） |
| `_reverse*.js` | **反向测试族**（`_` 前缀，自包含：备份 → 注入 → 跑套件 → **无论跑成什么样都还原** → 核 sha1）。每加一套新套件就该配一套。⚠ 判据不是「红了」而是**四件事同时成立**：① 注入点在产品里**唯一**；② 注入后**页面还解析得动**（否则那一针打的是语法、不是逻辑 —— 看起来像「所有断言都失败」）；③ **恰好**预期那几条红；④ **对照组一条不红**。⚠ 每条的 `red` / `green` 名单是**手写的**，所以它反过来会抓出套件的洞（第十九轮：`[].every()` 让一条断言**天生为真**、连带红没写进名单、注入点因重构消失）。⚠ 单跑 `node _verify/_reverse16.js` 约 2 分钟；它跑着的时候**别改产品**（见 §8 第 11 条）。⚠ 谁护着谁：`_reverse13` → `persona-verify` · `_reverse14` → `apig-verify`（第二十一轮迁到异步 `spawn`；⚠ 迁的时候发现它 **R4 的 `from` 把产品的报错文案一起写死了**，第十四段改文案后那一针已经**命中 0 次**、静默变成空操作 —— 详见下面那条） · `_reverse15` → `vertex-verify` · `_reverse16` → 第十九轮 Vertex 收口（**同时打 `vertex-verify` 和 `apig-verify` 两套** —— 一条产品逻辑有两份实现时，探针要**逐针点名跑哪一套**，否则另一份实现被改坏了也看不见）· `_reverse17` → 第二十轮（`[hidden]` 全局兜底 / `publisherModels` 分支 / 列模型候选顺序 / 列不出来要退回内置候选 / `needModel:false`，**同样两套都打**）· `_reverse18` → 第二十一轮「首页」（**往 `ST_TABS` 最前面插一个探针选项卡** / 把正文字段数写死 / 改成替换而不是追加 / 不写 `dataset.mode` / 不 clone 来源对象）。⚠⚠ 它第一版有 **3 处探针缺陷**：① `from` 以 `\n` 收尾而产品是 **CRLF** ⇒ 命中 0 次 ⇒ 注入是**空操作**（表现是和基线**一模一样**的 `60/0`，和「断言太弱」长得一样）；② 「把 `home` 删掉」会让 `#st-tab-home` 变 `null`，而套件里那句 `.click()` 会抛异常、`ev()` 又**直接 throw** ⇒ **整套崩掉、连汇总行都没有**；③ 连带红没列全。⚠⚠ 但那一轮**绝不是白跑**：R5 反过来抓到套件里一条**永真断言**（断言前又 `feedFile` 一次，`parseWorldBook` parse 出的是**全新对象** ⇒ 比的是两个不同对象 ⇒ 删掉 clone 也照样绿）—— 这正是反向测试的价值：**它是唯一能把「注入没效果」和「断言太弱」分开的东西**。见 `RULES.md` 六之四十七 |

> ⚠⚠ **现状（2026-09-24）**：`_reverse2` … `_reverse17` 这批老的**仍然用 `spawnSync`**，
> 而本机现在**同步 API 一律 `EBUSY`**（见 `RULES.md` 六之四十六）⇒ 它们会**假红**：
> 打出来是「基线没跑完 / 预期该红的没红」，看着像产品坏了，其实是起不了子进程。
> `_reverse18` 是**第一套改用异步 `spawn`** 的；**第二十一轮把 `_reverse14` 也迁了**
> （它的 `spawnSync` 只有一处 —— `runSuite()`，两个调用点，主流程包进 async IIFE 就行）。
> 剩下 **14 套**（`_reverse2/3/5/6/7/8/9/10/11/12/13/15/16/17`）还是同步 API。
> 要用老的那些，先把里面的 `spawnSync` 换成异步 `spawn`（`_reverse18.js` 开头那个 `run()`
> 就是现成写法；`_reverse14.js` 现在也是同一形状，可以互相照抄）。
>
> ⚠⚠ **迁 `_reverse14` 的副产品：抓到它 R4 那一针早就死了。** 它的 `from` 把产品的**报错文案**
> 一起写死了（`throw new Error('这个接口没返回任何模型');`），而第十四段把文案改成了
> 「返回里没有模型（字段名对不上？）」⇒ 命中 **0 次** ⇒ 那一针**静默变成空操作**。
> ⚠ 闸门是好的（会报「命中 0 次」，不会伪装成「没注入也全绿」），**但闸门只在有人跑它的时候
> 才起作用** —— 而它用的是 `spawnSync`，本机根本跑不起来，于是漂了整整一轮没人知道。
> ⇒ 教训：**「注入点唯一」这道闸门挡不住「没人跑」**；**写死产品文案的探针尤其脆** ——
> 产品改一句人话，探针就死了，而产品那边看不出任何异常。
>
> ⚠⚠ **外部 harness 那两套也一样**（`…\2026-09-17-00-10-39\_verify\` 里的
> `_reverse-tv.js` 与 `_reverse4.js`，同样 `const { spawnSync } = require('child_process')`）
> —— 第二十一轮**没有改它们**（不属当轮工作流）。要用就先照 `_reverse18.js` 那个 `run()` 改。
> ⚠ 但有一条**本轮核对过、结论是「不影响」**：`_reverse-tv.js` 的 `red`/`green` 名单里
> **没有任何带「选项卡」字样的断言名**，所以本轮改掉的那几条
> （`左侧有 15 个选项卡` → `16` / `默认选中「名字」` → `「首页」` / `15 个选项卡都能切过去` → `16`）
> **不会让它的名单漂掉** —— 这类「改了断言名要回头核反向测试名单」的核对，
> 结论无论是「要改」还是「不用改」都该写下来。
> ⚠ 但有一条**本轮核对过、结论是「不影响」**：`_reverse-tv.js` 的 `red`/`green` 名单里
> **没有任何带「选项卡」字样的断言名**，所以本轮改掉的那几条
> （`左侧有 15 个选项卡` → `16` / `默认选中「名字」` → `「首页」` / `15 个选项卡都能切过去` → `16`）
> **不会让它的名单漂掉** —— 这类「改了断言名要回头核反向测试名单」的核对，
...
> ✅ **复量（2026-09-23）**：16 套全跑一遍（3 m 48 s —— 那次 `apig-verify.js` 还没加，现在是 19 套）
> + 反向测试再跑 6 次，`cdp-*` 前缀
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 19 套，共 2 709 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
...
9. ⚠ **加服务商只改 `AI_PROVIDERS` 一张表**（`ST_AI_PROVIDERS` 是它的别名）。
   三条硬规矩：`custom` **必须留末位**（`stAiProviderOf` 的兜底）；
   **除 `custom` 外每项都要写死 `proto`**（它的留空是「按 URL 猜」，不是漏写）；
   新组名要进 `AI_PROVIDER_GROUPS`，否则会排到最后（不会丢，只是难看）。
   ⚠ **别给「端点不固定」的条目写死 `proto`** —— 写死了，resolver 就不再看 URL 了。
10. ⚠⚠ **改套件时先跑 `node _verify/_lint-suites.js`**。它扫的是「**模板字符串里出现反引号**」：
    `ev()` 的参数是模板字符串，注释里写一个 `` `名字` `` 就会把它提前结束，
    报的是**模板字符串开头那一行**的语法错，离现场很远。这个坑一轮之内踩了**三次**。
11. ⚠⚠ **别在反向测试跑着的时候改产品**。`_reverse*.js` 启动时把产品读进内存当基线，
    每个探针跑完从那份内存写回磁盘 ⇒ 你中途的改动会被下一次还原**盖掉**。
    非要改就先停掉它，改完**回读一次**。中途 kill 会留下 `_verify/_reverse*.bak`
    （里面是干净基线）**和一份被注入过的产品** —— 下次启动时套件会拒绝跑（那是对的），
    人工先 `cp` 回来再删 `.bak`。
