...
│                               ⚠ 真品判据：**1 495 761 字节 / 29 243 行**（`sha1 98ecb1b19fce…`）。
│                                 旁边那几份「副本」字节数都很接近，别靠文件名认。
│                                 ⚠ **这个数会漂** —— 本行换过好几轮
│                                    （`1 467 029 / 65df7b0b` → `1 487 774 / ecb46ca8de99`
│                                     → `1 493 542 / d0bf0d34` → 现在这份），
...
### 编写器专项：`G:\saki\_verify\`（16 个，共 2 245 条）
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 16 套，共 2 245 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
