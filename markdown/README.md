...
│                               ⚠ 真品判据：**1 550 630 字节 / 30 247 行**（`sha1 f48bd9a507c9…`）。
│                                 行数是 `grep -c ''` 口径（末行 `</html>` 后没有换行，所以 `wc -l` 会报 30 246）。
...
├── _verify\                  ← 验证 harness（17 套 + 探针，见 §7）
├── fonts\
...
| 行号 | 区块 | id / 类 |
|---|---|---|
| 5510 | **手机比例下的入口菜单**（窄屏 / 极扁窗口才出现） | `#mobile-nav` `.mobile-nav` |
| 5529 | 时钟卡片 | `#clock-tz-card` `.clock-container` |
| 5548 | 交互式日历卡片（默认折叠） | `#calendar-card` `.calendar-container.collapsed` |
| 5574 | AI 对话区域 | `#ai-card` `.ai-container` |
| 5833 | SillyTavern 角色卡编写器（折叠入口卡片） | `#st-card` `.st-container` |
| 5873 → 5874 | 编写器全屏浮层 | `#st-overlay` → `#st-shell` |
| 5900 | **🔌 API 全局配置按钮**（在小游戏按钮**上方**） | `#api-global-btn` `.game-entry-btn` |
| 5905 | 小游戏入口按钮 | `#game-entry-btn` |
| 5910 | 贴贴按钮（外链） | `.hug-button` |
| 5915 | 小游戏 Modal | `#gameModal` |
| 5990 | **🔌 API 全局配置 Modal** | `#apiGlobalModal` |
...
> `#gameCard` **5916** · `#gameShooterWrap` **5979** · `#gameShooterFrameHost` **5980**。
>
> ⚠⚠ **2026-09-23（人设生成器「三选一弹窗」那一轮）又往 `<style>` 末尾加了 65 行** ⇒
> 上面九行 + 这三个锚点**整张 +65**。⚠ **加在 `<style>` 末尾并不是零位移** ——
> 往 `<style>` 里插东西，`</style>`（现在在 **5302**）**之后**的锚点照样整体后移。
> 产品里那句「放末尾 = 零位移」的注释**是错的**（已改），这一轮就是它把 §2 九行全带偏的。
> **别按加法推，重跑 `_audit-anchors.js`。**
...
| 109 | 小游戏入口按钮 |
| 316 | 顶部标题 |
| 325 | 毛玻璃卡片基类 `.glass-card` |
| 338 | 时钟容器 |
| 356 / 403 | 时区切换开关 / 时区按钮网格 |
| 468 | 数字时钟 |
| 483 | 日历容器 |
| 720 | **AI 对话区域** |
| 867 | 模型设置子面板 |
| 943 | 流式 / 非流式开关 |
| 978 | **SillyTavern 风格 `{{user}}` 宏设置** |
| 1091 | 系统提示词预设 |
| 1140 | **酒馆模式 · 角色卡 / 世界书** |
| 1194 | 可折叠字段 |
| 1345 / 1347 | 来源细类配色（这条系统消息从哪来）/「第一段」单独配色 |
| 1388 | 世界书全局设置 |
| 1448 | 条目本轮会不会插入 |
| 1472 | 提示词预设：槽位列表 + 排序 / 编辑 |
| 1660 | 温度滑块 |
| 1676 | 聊天界面 |
| 1730 | 消息行：气泡 + 重试按钮 |
| 1826 | 贴贴按钮 |
| 1858 | Modal 弹窗 |
| 1941 / 1990 / 2008 / 2018 | 游戏列表 / Canvas / 触屏按键 / 十字键 |
| 2084 | **复古线框战机**（整页游戏：卡片放宽档 `.game-card--wide` + iframe 容器） |
| 2111 | **复古模式覆盖** |
| 2413 | 酒馆模式 · 复古皮肤 |
| 2576 | **SillyTavern 角色卡编写器**（多行大横幅） |
| 2665 / 2775 / 2842 | 全屏浮层 / 左侧选项卡 / 右侧内容 |
| 3089 | 条目 / 脚本卡片 |
| 3166 | MVU 变量框架 |
| 3370 | MVU 状态栏外观编辑器 |
| 3890 | 浮层复古皮肤 |
| 4091 | 编写器里的 AI 助手 |
| 4439 | **Code（Agent）页**（多行大横幅） |
| 5023 | Code 页 · 复古皮肤 |
| 5125 | **AI 聊天窗口「放大」模式**（多行大横幅） |
| 5142 / 5154 / 5210 | 遮罩 / 撑满高度 / 桌面居中 |
| 5238 | **手机比例下的入口菜单**（多行大横幅） |
| 5365 | **复古模式：手机菜单照 Win98 / Win2000**（多行大横幅） |
...
### `apiGlobal` / `aiConfig.own`（API 全局配置，`apiGlobal` 在 7510 行、`aiConfig` 在 7395 行）

| 对象 | 字段 |
|---|---|
| `apiGlobal`（存 `apiGlobalCfg`） | `provider` / `baseUrl` / `apiKey` / `model` / `models`（拉过的列表缓存）/ `temperature` / `maxTokens` / `stream` / `firstSeg` —— **全站唯一真相源**，只有主页【🔌 API 全局配置】那个弹窗改它 |
| `aiConfig.followGlobal` | 布尔，默认 `true`。【ai对话】的「配置来源」：`true` = 跟随 `apiGlobal`，`false` = 用 `aiConfig.own` |
| `aiConfig.own` | 【ai对话】切到「独立配置」时自己那一套（字段同 `apiGlobal`，**没有** `models` 缓存）。⚠ 它跟 `apiGlobal` **互不影响** |
...
### `stAi` / `stAiUi`（编写器 AI 助手，`stAi` 在 16457 行、`stAiUi` 在 17079 行）
...
它是留给「单独跑某一层」的时候用的。另外 `G:\saki\_verify\` 那 17 套是**零依赖**的，不用 `NODE_PATH`。）
...
⚠ 也**不写死期望总数**（理由同上）。
⚠ **本地那 17 套不需要这第四条**：它们的正则 `N 通过 / M 失败` 要求两边都是数字，
...
>
> ⚠ 这个目录的收尾格式跟 `_verify/` 那 17 套**又不一样**：这里是 `166/166 通过`
...
### 编写器专项：`G:\saki\_verify\`（17 个，共 2 428 条）
...
cd /g/saki/_verify
N="C:/Users/YanSaki/.workbuddy-ai/binaries/node/versions/22.22.2-2/node.exe"
"$N" run-all.js                # 全部 17 套：逐套打印 + 合计 + 清单对账
...
这个目录里除了上面 17 套，还有几类**不是回归测试**的文件，别被名字绕进去：
...
| `run-all.js` | **跑全部 17 套的 runner**，带「清单对账 + 汇总行解析 + 合计」。⚠ 它自己也在清单里（`NOT_A_SUITE`），否则会被自己对账拦下 |
...
| `_fix-anchors.js` | **`_audit-anchors.js` 的取真值搭档**（`_` 前缀，只读）：审计只说「哪几行对不上」，它负责**把真值算出来**。§2 走审计那套判据直接取值；§3 走**可验证的位移** `node _fix-anchors.js <位移>`（默认 194），逐行核「旧值 + 位移」那一行**是不是横幅**（`/*` 开头），不是就打 ❌ 并列出来。⚠ 位移**每轮都不一样**（取决于这轮插了几行、插在哪），**必须现量**；拿不准就把位移传错 —— 它会**大声报 ❌**，不会静默给错值。⚠ 早先那版「单调对齐 + 共享字」的启发式**实测会错**（一次错 2 行）⇒ 已弃用。⚠ 它输出里每行都带横幅原文，**要人核一眼**再往 §2 / §3 里抄 |
...
> ✅ **复量（2026-09-23）**：16 套全跑一遍（3 m 48 s —— 那次 `apig-verify.js` 还没加，现在是 17 套）
> + 反向测试再跑 6 次，`cdp-*` 前缀
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 17 套，共 2 428 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
