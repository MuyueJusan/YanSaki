...
│                               ⚠ 真品判据：**1 515 017 字节 / 29 532 行**（`sha1 b3149a58ea13…`）。
│                                 行数是 `grep -c ''` 口径（末行 `</html>` 后没有换行，所以 `wc -l` 会报 29 531）。
...
| 行号 | 区块 | id / 类 |
|---|---|---|
| 5316 | **手机比例下的入口菜单**（窄屏 / 极扁窗口才出现） | `#mobile-nav` `.mobile-nav` |
| 5335 | 时钟卡片 | `#clock-tz-card` `.clock-container` |
| 5354 | 交互式日历卡片（默认折叠） | `#calendar-card` `.calendar-container.collapsed` |
| 5380 | AI 对话区域 | `#ai-card` `.ai-container` |
| 5625 | SillyTavern 角色卡编写器（折叠入口卡片） | `#st-card` `.st-container` |
| 5665 → 5666 | 编写器全屏浮层 | `#st-overlay` → `#st-shell` |
| 5690 | 小游戏入口按钮 | `.game-entry-btn` |
| 5695 | 贴贴按钮（外链） | `.hug-button` |
| 5700 | 小游戏 Modal | `#gameModal` |

> 小游戏弹窗**里面**的三个新锚点（第二十四轮加的，不在上表里 —— 上表只收 body 顶层）：
> `#gameCard` **5701** · `#gameShooterWrap` **5764** · `#gameShooterFrameHost` **5765**。
>
...
### 编写器专项：`G:\saki\_verify\`（16 个，共 2 334 条）
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 16 套，共 2 334 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
