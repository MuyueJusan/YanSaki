...
| [04-card-editor.md](04-card-editor.md) | SillyTavern 角色卡编写器（全屏浮层，14 个选项卡） |
...
├── _verify\                  ← 验证 harness（15 套 + 探针，见 §7）
├── fonts\
...
| 行号 | 区块 | id / 类 |
|---|---|---|
| 5193 | **手机比例下的入口菜单**（窄屏 / 极扁窗口才出现） | `#mobile-nav` `.mobile-nav` |
| 5212 | 时钟卡片 | `#clock-tz-card` `.clock-container` |
| 5231 | 交互式日历卡片（默认折叠） | `#calendar-card` `.calendar-container.collapsed` |
| 5257 | AI 对话区域 | `#ai-card` `.ai-container` |
| 5502 | SillyTavern 角色卡编写器（折叠入口卡片） | `#st-card` `.st-container` |
| 5541 → 5542 | 编写器全屏浮层 | `#st-overlay` → `#st-shell` |
| 5566 | 小游戏入口按钮 | `.game-entry-btn` |
| 5571 | 贴贴按钮（外链） | `.hug-button` |
| 5576 | 小游戏 Modal | `#gameModal` |

> 小游戏弹窗**里面**的三个新锚点（第二十四轮加的，不在上表里 —— 上表只收 body 顶层）：
> `#gameCard` **5577** · `#gameShooterWrap` **5640** · `#gameShooterFrameHost` **5641**。
...
| 3641 | 浮层复古皮肤 |
| 3839 | 编写器里的 AI 助手 |
| 4187 | **Code（Agent）页**（多行大横幅） |
| 4771 | Code 页 · 复古皮肤 |
| 4873 | **AI 聊天窗口「放大」模式**（多行大横幅） |
| 4890 / 4902 / 4958 | 遮罩 / 撑满高度 / 桌面居中 |
| 4986 | **手机比例下的入口菜单**（多行大横幅） |
| 5113 | **复古模式：手机菜单照 Win98 / Win2000**（多行大横幅） |

> 多行大横幅有好几处（2355 / 4160 / 4846 / 4959 / 5086）—— 它们是**多行**写法
> （`/* ====` 换行再写标题），单行横幅的提取脚本扫不到标题那一行，
> 所以 `_audit-anchors.js` 会取**整块**再找名字。
> 找它们要 `grep -n "SillyTavern 角色卡编写器\|Code（Agent）页\|手机比例下的入口菜单\|照 Win98"`。
>
> ⚠ **2355 之前的行号 2026-09-20 实测全部准确**；**之后的整段 +41**（编写器那段里插过两次 CSS）。
> ⚠ 一次能"推导"出来的位移，**别靠加法算** —— 插入点分散时每段的偏移量都不同
> （实测 2424 → 2444 是 +20，而 2580 → 2621 是 +41，**同一张表里两种位移**）。
> **一律重新 `grep`。**
...
它是留给「单独跑某一层」的时候用的。另外 `G:\saki\_verify\` 那 15 套是**零依赖**的，不用 `NODE_PATH`。）
...
⚠ 也**不写死期望总数**（理由同上）。
⚠ **本地那 15 套不需要这第四条**：它们的正则 `N 通过 / M 失败` 要求两边都是数字，
...
>
> ⚠ 这个目录的收尾格式跟 `_verify/` 那 15 套**又不一样**：这里是 `166/166 通过`
...
### 编写器专项：`G:\saki\_verify\`（15 个，共 2 056 条）
...
> （`smoke.js` 87 / `st-code.js` 670 / 视觉层 371 …，对不上就是少走了）。
...
这个目录里除了上面 15 套，还有几类**不是回归测试**的文件，别被名字绕进去：
...
| `run-all.js` | **跑全部 15 套的 runner**，带「清单对账 + 汇总行解析 + 合计」。⚠ 它自己也在清单里（`NOT_A_SUITE`），否则会被自己对账拦下 |
...
> 它们都是 `fs.mkdtempSync(path.join(os.tmpdir(), 'cdp-…-'))` 建出来、当 `--user-data-dir` 用，
> 收尾时**没有** `rmSync`。本地 15 套里 14 套走 CDP ⇒ **每跑一次全量就多 14 个目录**。
> 2026-09-20 审查时实测：`cdp-*` 前缀 **873 个目录、约 14.5 GB**。**这是跑验证的固定成本。**
>
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 15 套，共 2 056 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
