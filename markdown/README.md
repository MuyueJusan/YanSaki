...
│                               ⚠ 真品判据：**1 594 295 字节 / 30 892 行**（`sha1 47e78fa4a57f…`）。
│                                 行数是 `grep -c ''` 口径（末行 `</html>` 后没有换行，所以 `wc -l` 会报 30 891）。
...
| 行号 | 区块 | id / 类 |
|---|---|---|
| 5530 | **手机比例下的入口菜单**（窄屏 / 极扁窗口才出现） | `#mobile-nav` `.mobile-nav` |
| 5549 | 时钟卡片 | `#clock-tz-card` `.clock-container` |
| 5568 | 交互式日历卡片（默认折叠） | `#calendar-card` `.calendar-container.collapsed` |
| 5594 | AI 对话区域 | `#ai-card` `.ai-container` |
| 5866 | SillyTavern 角色卡编写器（折叠入口卡片） | `#st-card` `.st-container` |
| 5906 → 5907 | 编写器全屏浮层 | `#st-overlay` → `#st-shell` |
| 5933 | **🔌 API 全局配置按钮**（在小游戏按钮**上方**） | `#api-global-btn` `.game-entry-btn` |
| 5938 | 小游戏入口按钮 | `#game-entry-btn` |
| 5943 | 贴贴按钮（外链） | `.hug-button` |
| 5948 | 小游戏 Modal | `#gameModal` |
| 6023 | **🔌 API 全局配置 Modal** | `#apiGlobalModal` |
...
> `#gameCard` **5949** · `#gameShooterWrap` **6012** · `#gameShooterFrameHost` **6013**。
>
> ⚠⚠ **2026-09-23（第十八轮：服务商扩充 + Vertex AI）这一轮 `</style>` 之后的锚点整体 +33**，
> 而且是**两笔**：样式块里插了两处（Vertex 那块样式 + 它的复古覆盖）净增 **+20**
> （`</style>` 现在在 **5516**）；正文里又加了两块 —— 【ai对话】的模型框从死下拉改成
> `input` + `datalist`、以及独立模式 + Vertex 时那句交代 —— 再 **+13**。
> ⚠ **前四行（5530 / 5549 / 5568 / 5594）没动**，因为那两块加在 `#ai-card` **里面**
> ⇒ **同一张表里两段位移并存**，加法的第一个数就错了。**一律重跑 `_audit-anchors.js`。**
...
| 109 | 小游戏入口按钮 |
| 336 | 顶部标题 |
| 345 | 毛玻璃卡片基类 `.glass-card` |
| 358 | 时钟容器 |
| 376 / 423 | 时区切换开关 / 时区按钮网格 |
| 488 | 数字时钟 |
| 503 | 日历容器 |
| 740 | **AI 对话区域** |
| 887 | 模型设置子面板 |
| 963 | 流式 / 非流式开关 |
| 998 | **SillyTavern 风格 `{{user}}` 宏设置** |
| 1111 | 系统提示词预设 |
| 1160 | **酒馆模式 · 角色卡 / 世界书** |
| 1214 | 可折叠字段 |
| 1365 / 1367 | 来源细类配色（这条系统消息从哪来）/「第一段」单独配色 |
| 1408 | 世界书全局设置 |
| 1468 | 条目本轮会不会插入 |
| 1492 | 提示词预设：槽位列表 + 排序 / 编辑 |
| 1680 | 温度滑块 |
| 1696 | 聊天界面 |
| 1750 | 消息行：气泡 + 重试按钮 |
| 1846 | 贴贴按钮 |
| 1878 | Modal 弹窗 |
| 1961 / 2010 / 2028 / 2038 | 游戏列表 / Canvas / 触屏按键 / 十字键 |
| 2104 | **复古线框战机**（整页游戏：卡片放宽档 `.game-card--wide` + iframe 容器） |
| 2131 | **复古模式覆盖** |
| 2433 | 酒馆模式 · 复古皮肤 |
| 2596 | **SillyTavern 角色卡编写器**（多行大横幅） |
| 2685 / 2795 / 2862 | 全屏浮层 / 左侧选项卡 / 右侧内容 |
| 3109 | 条目 / 脚本卡片 |
| 3186 | MVU 变量框架 |
| 3390 | MVU 状态栏外观编辑器 |
| 3910 | 浮层复古皮肤 |
| 4111 | 编写器里的 AI 助手 |
| 4459 | **Code（Agent）页**（多行大横幅） |
| 5043 | Code 页 · 复古皮肤 |
| 5145 | **AI 聊天窗口「放大」模式**（多行大横幅） |
| 5162 / 5174 / 5230 | 遮罩 / 撑满高度 / 桌面居中 |
| 5258 | **手机比例下的入口菜单**（多行大横幅） |
| 5385 | **复古模式：手机菜单照 Win98 / Win2000**（多行大横幅） |

> 多行大横幅有好几处（2576 / 4439 / 5125 / 5238 / 5365）—— 它们是**多行**写法
...
### 编写器专项：`G:\saki\_verify\`（18 个，共 2 558 条）
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 18 套，共 2 558 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
