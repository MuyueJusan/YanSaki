...
│                               ⚠ 真品判据：**1 620 220 字节 / 31 338 行**（`sha1 ef62eba8edb3…`）。
│                                 行数是 `grep -c ''` 口径（末行 `</html>` 后没有换行，所以 `wc -l` 会报 31 337）。
...
├── _verify\                  ← 验证 harness（18 套 + 探针，见 §7）
├── fonts\
...
| 行号 | 区块 | id / 类 |
|---|---|---|
| 5588 | **手机比例下的入口菜单**（窄屏 / 极扁窗口才出现） | `#mobile-nav` `.mobile-nav` |
| 5607 | 时钟卡片 | `#clock-tz-card` `.clock-container` |
| 5626 | 交互式日历卡片（默认折叠） | `#calendar-card` `.calendar-container.collapsed` |
| 5652 | AI 对话区域 | `#ai-card` `.ai-container` |
| 5924 | SillyTavern 角色卡编写器（折叠入口卡片） | `#st-card` `.st-container` |
| 5964 → 5965 | 编写器全屏浮层 | `#st-overlay` → `#st-shell` |
| 5991 | **🔌 API 全局配置按钮**（在小游戏按钮**上方**） | `#api-global-btn` `.game-entry-btn` |
| 5996 | 小游戏入口按钮 | `#game-entry-btn` |
| 6001 | 贴贴按钮（外链） | `.hug-button` |
| 6006 | 小游戏 Modal | `#gameModal` |
| 6081 | **🔌 API 全局配置 Modal** | `#apiGlobalModal` |
...
> `#gameCard` **6007** · `#gameShooterWrap` **6070** · `#gameShooterFrameHost` **6071**。
>
> ⚠⚠ **2026-09-23（第十九轮：Vertex AI 收口）这一轮是「一段位移」：整张表 +58**
> （`</style>` 从 5516 挪到 **5574**）。这 58 行**全部来自样式块**（SA 掩码条 / 验证结果区 /
> 它俩的复古覆盖）；正文那边虽然也给弹窗加了 HTML（掩码条 + 验证按钮 + 结果区），
> 但都加在 `#apiGlobalModal` **里面** ⇒ 表里只有弹窗内那三个锚点额外多漂一截
> （`#gameCard` 是 +58 整、`#gameShooterWrap` / `#gameShooterFrameHost` 也是 +58 ——
> 因为新 HTML 在它们**之后**）。**⚠ 结论照旧：位移形状每轮都不一样，
> 「我插了多少行」推不出位移，一律重跑 `_audit-anchors.js`。**
...
| 109 | 小游戏入口按钮 |
| 394 | 顶部标题 |
| 403 | 毛玻璃卡片基类 `.glass-card` |
| 416 | 时钟容器 |
| 434 / 481 | 时区切换开关 / 时区按钮网格 |
| 546 | 数字时钟 |
| 561 | 日历容器 |
| 798 | **AI 对话区域** |
| 945 | 模型设置子面板 |
| 1021 | 流式 / 非流式开关 |
| 1056 | **SillyTavern 风格 `{{user}}` 宏设置** |
| 1169 | 系统提示词预设 |
| 1218 | **酒馆模式 · 角色卡 / 世界书** |
| 1272 | 可折叠字段 |
| 1423 / 1425 | 来源细类配色（这条系统消息从哪来）/「第一段」单独配色 |
| 1466 | 世界书全局设置 |
| 1526 | 条目本轮会不会插入 |
| 1550 | 提示词预设：槽位列表 + 排序 / 编辑 |
| 1738 | 温度滑块 |
| 1754 | 聊天界面 |
| 1808 | 消息行：气泡 + 重试按钮 |
| 1904 | 贴贴按钮 |
| 1936 | Modal 弹窗 |
| 2019 / 2068 / 2086 / 2096 | 游戏列表 / Canvas / 触屏按键 / 十字键 |
| 2162 | **复古线框战机**（整页游戏：卡片放宽档 `.game-card--wide` + iframe 容器） |
| 2189 | **复古模式覆盖** |
| 2491 | 酒馆模式 · 复古皮肤 |
| 2654 | **SillyTavern 角色卡编写器**（多行大横幅） |
| 2743 / 2853 / 2920 | 全屏浮层 / 左侧选项卡 / 右侧内容 |
| 3167 | 条目 / 脚本卡片 |
| 3244 | MVU 变量框架 |
| 3448 | MVU 状态栏外观编辑器 |
| 3968 | 浮层复古皮肤 |
| 4169 | 编写器里的 AI 助手 |
| 4517 | **Code（Agent）页**（多行大横幅） |
| 5101 | Code 页 · 复古皮肤 |
| 5203 | **AI 聊天窗口「放大」模式**（多行大横幅） |
| 5220 / 5232 / 5288 | 遮罩 / 撑满高度 / 桌面居中 |
| 5316 | **手机比例下的入口菜单**（多行大横幅） |
| 5443 | **复古模式：手机菜单照 Win98 / Win2000**（多行大横幅） |

> 多行大横幅有好几处（2654 / 4517 / 5203 / 5316 / 5443）—— 它们是**多行**写法
...
它是留给「单独跑某一层」的时候用的。另外 `G:\saki\_verify\` 那 18 套是**零依赖**的，不用 `NODE_PATH`。）
...
⚠ 也**不写死期望总数**（理由同上）。
⚠ **本地那 18 套不需要这第四条**：它们的正则 `N 通过 / M 失败` 要求两边都是数字，
...
>
> ⚠ 这个目录的收尾格式跟 `_verify/` 那 18 套**又不一样**：这里是 `166/166 通过`
...
### 编写器专项：`G:\saki\_verify\`（18 个，共 2 607 条）
...
cd /g/saki/_verify
N="C:/Users/YanSaki/.workbuddy-ai/binaries/node/versions/22.22.2-2/node.exe"
"$N" run-all.js                # 全部 18 套：逐套打印 + 合计 + 清单对账
...
这个目录里除了上面 18 套，还有几类**不是回归测试**的文件，别被名字绕进去：
...
| `run-all.js` | **跑全部 18 套的 runner**，带「清单对账 + 汇总行解析 + 合计」。⚠ 它自己也在清单里（`NOT_A_SUITE`），否则会被自己对账拦下 |
...
| `_lint-suites.js` | **改完套件先跑这个**（`_` 前缀）：把 `_verify/*.js` 逐个过 `node --check`，语法不通就指名道姓报出来。⚠ 为什么值得单开：套件里到处是 `ev(\`…\`)` 这种**模板字符串**，注释里只要出现**一个反引号**（比如写「\`custom\` 那条」），字符串就提前结束，报的是 `SyntaxError: missing ) after argument list` —— **指向的行号是模板字符串开头**，看着跟那句注释毫无关系，人眼扫不出来。⚠ 第十八轮**连踩三次**（`vertex-verify.js` 两次、`apig-verify.js` 一次），所以做成闸门 |
| `_reverse*.js` | **反向测试族**（`_` 前缀，自包含：备份 → 注入 → 跑套件 → **无论跑成什么样都还原** → 核 sha1）。每加一套新套件就该配一套。⚠ 判据不是「红了」而是**四件事同时成立**：① 注入点在产品里**唯一**；② 注入后**页面还解析得动**（否则那一针打的是语法、不是逻辑 —— 看起来像「所有断言都失败」）；③ **恰好**预期那几条红；④ **对照组一条不红**。⚠ 每条的 `red` / `green` 名单是**手写的**，所以它反过来会抓出套件的洞（第十九轮：`[].every()` 让一条断言**天生为真**、连带红没写进名单、注入点因重构消失）。⚠ 单跑 `node _verify/_reverse16.js` 约 2 分钟；它跑着的时候**别改产品**（见 §8 第 11 条）。⚠ 谁护着谁：`_reverse13` → `persona-verify` · `_reverse14` → `apig-verify` · `_reverse15` → `vertex-verify` · `_reverse16` → 第十九轮 Vertex 收口（**同时打 `vertex-verify` 和 `apig-verify` 两套** —— 一条产品逻辑有两份实现时，探针要**逐针点名跑哪一套**，否则另一份实现被改坏了也看不见） |
...
> ✅ **复量（2026-09-23）**：16 套全跑一遍（3 m 48 s —— 那次 `apig-verify.js` 还没加，现在是 18 套）
> + 反向测试再跑 6 次，`cdp-*` 前缀
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 18 套，共 2 607 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
