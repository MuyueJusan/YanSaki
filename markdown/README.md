...
| 行号 | 区块 | id / 类 |
|---|---|---|
| 5594 | **手机比例下的入口菜单**（窄屏 / 极扁窗口才出现） | `#mobile-nav` `.mobile-nav` |
| 5613 | 时钟卡片 | `#clock-tz-card` `.clock-container` |
| 5632 | 交互式日历卡片（默认折叠） | `#calendar-card` `.calendar-container.collapsed` |
| 5658 | AI 对话区域 | `#ai-card` `.ai-container` |
| 5930 | SillyTavern 角色卡编写器（折叠入口卡片） | `#st-card` `.st-container` |
| 5970 → 5971 | 编写器全屏浮层 | `#st-overlay` → `#st-shell` |
| 5997 | **🔌 API 全局配置按钮**（在小游戏按钮**上方**） | `#api-global-btn` `.game-entry-btn` |
| 6002 | 小游戏入口按钮 | `#game-entry-btn` |
| 6007 | 贴贴按钮（外链） | `.hug-button` |
| 6012 | 小游戏 Modal | `#gameModal` |
| 6087 | **🔌 API 全局配置 Modal** | `#apiGlobalModal` |
...
> `#gameCard` **6013** · `#gameShooterWrap` **6076** · `#gameShooterFrameHost` **6077**。
>
...
| 3974 | 浮层复古皮肤 |
| 4175 | 编写器里的 AI 助手 |
| 4523 | **Code（Agent）页**（多行大横幅） |
| 5107 | Code 页 · 复古皮肤 |
| 5209 | **AI 聊天窗口「放大」模式**（多行大横幅） |
| 5226 / 5238 / 5294 | 遮罩 / 撑满高度 / 桌面居中 |
| 5322 | **手机比例下的入口菜单**（多行大横幅） |
| 5449 | **复古模式：手机菜单照 Win98 / Win2000**（多行大横幅） |

> 多行大横幅有好几处（2660 / 4523 / 5209 / 5322 / 5449）—— 它们是**多行**写法
...
### 编写器专项：`G:\saki\_verify\`（19 个，共 2 691 条）
...
1. **改完必须跑全量 8 层**（再加上 `_verify/` 那 19 套，共 2 691 条）。这套验证不是装饰，历史上有多次「改一处碎三处」都是它抓出来的。
