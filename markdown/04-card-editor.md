...
> 见 02 的「手机比例下的入口菜单」。
>
> 功能：新建 / 编辑 / 改写角色卡，导出 PNG 或 JSON。左侧 **14 个选项卡**，可折叠。
...
| 14 | `export` | 💾 | 导出 | PNG / JSON + JSON 预览 + 自检 |
...
真实 Chrome 里另有 `verify_tavern_visual.js` 的第 12 / 12b / 12c / 12d 段（浮层几何、十四个页签切换、移动端触控、复古皮肤、MVU 页）。
...
（选项卡 / 行 / 列）之后，除了 grep 数量，还要 grep 字面量数组。**
...
**`_verify/smoke.js`** —— 全应用冒烟，**45 条**：14 个选项卡逐个切换都能渲染、
...
| I | 面板：14 个选项卡、AI 页渲染、两个面板挂得上、复古皮肤下方角 |
...
**`_verify/st-code.js`** —— 「**Code（Agent）页**」，**663 条**：
...
| 段 | 内容 |
|---|---|
| A | 接线与布局：14 个选项卡里有 `code`、左右两栏、工具条的折叠菜单与动作按钮、窄屏分段控件 |
