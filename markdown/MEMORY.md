...
- ⚠ **「发生了」必须配「不该发生」的对照组**，否则可能**永远为真**；⚠⚠ **但对照组必须「不同」** —— A 与 B 碰巧同串时两条取值路径读出同一个值，**删掉实现照样全绿**（六之二十六）。⚠⚠ **「取不到就换一个」的回落 = 把失败伪装成成功** ⇒ **回落必须说出来**（六之二十七）。⚠ **断言方向本身就是需求** ⇒ 改主意**先改断言、看它红，再改产品**。⚠ **数组不能用 `===` 比**；`check(name, actual, pred, expect)` **只收 4 个参数**；**折叠面板的「看不见」rect 量不出** ⇒ 用命中测试。
- ⚠⚠ **「跑完了」≠「每条都跑到了」**（提前 `process.exit()` 静默吃掉后面）；⚠ **「从来没发生过」要拿当时的快照支撑**，不能靠回忆 ⇒ **锚点一律选结构性的**。
- ⚠⚠ **写死清单 / 写死数字每次加东西都会来收账** ⇒ **改推导式 / 跟基线比**（最阴：遍历清单是字面量数组 ⇒ **根本不红、只静默少走**）。⚠⚠ **顺序型断言有三种静默形态**（六之十七），**都不会当场红**，必须人工核。⚠ **探针造的文件无条件删**；**改了断言名要重跑对着它的反向测试**。⚠⚠ **新断言要崩不掉** —— 注入后抛异常会让套件**当场炸掉、连汇总行都没有**，反向测试拿不到「红了几条」，那一针**等于白跑** ⇒ 读数组元素一律 `(a[i] || {})`。
...
- **两处 `_verify`**：`G:\saki\_verify\`（**16 套 / 2 291 条**，零依赖）+ `C:\Users\YanSaki\WorkBuddy AI\2026-09-17-00-10-39\_verify\`（8 层 / 1 631 条，要 `NODE_PATH=…/node/workspace/node_modules`）。⚠ 同名不同地，各用各的 `run-all.js`。
