...
- ⚠⚠ **写死清单 / 写死数字每次加东西都会来收账** ⇒ **改推导式 / 跟基线比**（最阴：遍历清单是字面量数组 ⇒ **根本不红、只静默少走**）。⚠⚠ **顺序型断言有三种静默形态**（六之十七），**都不会当场红**，必须人工核。⚠ **探针造的文件无条件删**；**改了断言名要重跑对着它的反向测试**。⚠⚠ **新断言要崩不掉** —— 注入后抛异常会让套件**当场炸掉、连汇总行都没有**，反向测试拿不到「红了几条」，那一针**等于白跑** ⇒ 读数组元素一律 `(a[i] || {})`。⚠⚠ **注入后的连带红要一条不落地写进 `red`**（不然「没有预期之外的红」闸门拦下）；⚠ **「对照」不等于「必然绿」**（六之二十九）。
