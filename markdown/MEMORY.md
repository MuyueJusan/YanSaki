...
- ⚠⚠ **写死清单 / 写死数字每次加东西都会来收账** ⇒ **改推导式 / 跟基线比**（最阴：遍历清单是字面量数组 ⇒ **根本不红、只静默少走**）。⚠⚠ **顺序型断言有三种静默形态**（六之十七），**都不会当场红**，必须人工核。⚠ **探针造的文件无条件删**；**改了断言名要重跑对着它的反向测试**。⚠⚠ **新断言要崩不掉** —— 注入后抛异常会让套件**当场炸掉、连汇总行都没有**，反向测试拿不到「红了几条」，那一针**等于白跑** ⇒ 读数组元素一律 `(a[i] || {})`。⚠⚠ **但「崩不掉」和「不会永远为真」会打架** —— `[].every()` / `[].some()` 对空数组**有定义好的返回值** ⇒ 光加 `|| []` 会让断言**天生为真**（字段被整个删掉照样绿）⇒ 一律改成**「数出来跟期望比」**（六之三十九）。⚠⚠ **注入后的连带红要一条不落地写进 `red`**（不然「没有预期之外的红」闸门拦下）；⚠ **「对照」不等于「必然绿」**（六之二十九）。
...
- ⚠⚠ **别在反向测试跑着的时候改产品** —— 它从内存基线写回磁盘，你的改动会被**静默盖掉**；kill 后留下 `.bak`（干净基线）+ **一份被注入过的产品**（`grep "注入："` 核），套件下次会拒绝跑（那是对的）（六之三十四）。⚠ **模板字符串里出现反引号**（哪怕在注释里）会把字符串提前结束，报错却指向**函数开头** ⇒ 改完套件先跑 `_verify/_lint-suites.js`（六之三十七）。⚠ **注入要窄到只打它声称打的那件事**（整块替换会把对照组一起打红）；⚠ **一份逻辑 N 份实现 ⇒ 探针逐针点名跑哪一套**（六之四十一 / 四十二）。⚠ **「函数写好了」≠「被调用了」** —— 静态检查与行为断言**都看不见**零调用点 ⇒ 覆盖盯**调用点**、拿场景走**真实请求路径**（六之四十）。
...
- **两处 `_verify`**：`G:\saki\_verify\`（**18 套** + 反向测试族 `_reverse*.js`，零依赖）+ `C:\Users\YanSaki\WorkBuddy AI\2026-09-17-00-10-39\_verify\`（8 层，要 `NODE_PATH=…/node/workspace/node_modules`）。⚠ **条数会每轮涨** ⇒ **现量 / 见 `README` §7 表**，别抄；同名不同地，各用各的 `run-all.js`。
...
| 主题（展开版在哪） | 最要命的那一条 |
|---|---|
| ② 编写器 / 预制块 / AI 预制块 —— 一 · 一之补 · 一之补二 | `stSet(path,v)` **只写标量叶子**（传错路径整块变 `''`）；块树递归「这层没子块」= 空列表，**绝不能兜底回根列表**；`blk` **先 extra、后 bind** |
