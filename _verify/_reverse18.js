...
//   node _reverse18.js --only R6       # 只跑基线 + 点名的针（补针时用，约 9 分钟而不是 30 分钟）
// ⚠ 实测**每针约 4.3 分钟**（`home-verify.js` 单次就是这个量级，CDP 起 Chrome 的开销），
//   6 针 + 基线 ≈ **30 分钟**。别按乐观估计排时间：单次在 900 s 的 timeout 之内，但**总时长会超**。
...
//   6 针全跑约 26 分钟（每针 = 一次完整套件运行 ≈ 4.3 分钟），而**补一针**时
//   只跑那一针能把这一轮压到 ~9 分钟。
